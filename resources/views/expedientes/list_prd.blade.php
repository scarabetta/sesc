...
@section('content')
        ...
        <h3 class="text-center">{{ ucfirst(Auth::user()->fondo->nombre) }}</h2>
                ...
                @foreach($expedientes as $expediente)
                    ...
                    $expediente->estadoActual() == App\Expediente::ESTADO_RECHAZADO_CIUDADANO)
                        ...
                        @endif
                          ...
                          <td>${{ number_format($expediente->prodanza_monto_total, 2, ',', '') }}</td>
                          <td>${{ number_format($expediente->prodanza_monto_solicitado, 2, ',', '') }}</td>
