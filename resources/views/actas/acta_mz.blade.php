...
<html>
    ...
    <body>
        <h3>Acta</h3>
        <p>
            En la Ciudad de Buenos Aires, a {{ $fecha_actual['dia'] }} días del mes de {{ $fecha_actual['mes'] }}  de {{ $fecha_actual['year'] }}, y siendo las {{ $fecha_actual['hora'] }} hs, se reúne el Consejo de Promoción Cultural de la Ciudad Autónoma de Buenos Aires con la presencia de: 
            @foreach($acta->users as $key=>$presente) {{ $presente->titulo }} {{ $presente->name }} {{ $presente->surename }}
            @if ($key < $jueces_total-2) , @endif 
            @if ($key == $jueces_total-2) y @endif @endforeach. 
        ...
        <p>
            <strong> 2. Proyectos No Declarados de Interés Cultural:</strong>
             ...
             @foreach($acta->expedientes as $key=>$expediente) 
              @if ($expediente->pivot->estado == $expedientes_estado_rechazado) Proyecto N° {{ $expediente->numero }} /RCP/ {{ date('Y',strtotime($expediente->fecha_presentacion)) }}, “{{ $expediente->proyecto_nombre }}” 
                @if($expediente->pivot->observaciones != '')
                  Observaciones: {{ $expediente->pivot->observaciones }};
                @endif
              @endif 
